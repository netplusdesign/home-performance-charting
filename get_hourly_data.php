...
WHERE CAST(ti1.date AS DATE) = DATE('2012-12-29')
	AND ti1.house_id = 0;
		...
		$query .= "LEFT JOIN (SELECT house_id, date, temperature AS 'indoor2_deg' FROM temperature_hourly WHERE device_id = 2) ti2 ON ti2.date = ti1.date AND ti2.house_id = ti1.house_id ";
		$query .= "LEFT JOIN (SELECT house_id, date, temperature AS 'indoor0_deg' FROM temperature_hourly WHERE device_id = 3) ti0 ON ti0.date = ti1.date AND ti0.house_id = ti1.house_id ";
		$query .= "LEFT JOIN (SELECT house_id, date, temperature AS 'outdoor_deg' FROM temperature_hourly WHERE device_id = 0) tu ON tu.date = ti1.date AND tu.house_id = ti1.house_id ";
		$query .= "LEFT JOIN (SELECT house_id, date, hdd FROM hdd_hourly) th ON th.date = ti1.date AND th.house_id = ti1.house_id ";
		$query .= "LEFT JOIN energy_hourly e ON e.date = ti1.date AND e.house_id = ti1.house_id ";
